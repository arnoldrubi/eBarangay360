...
try {
    // Insert the blotter report first
    $stmt = $pdo->prepare("
        INSERT INTO blotter_reports (
            ...
            attending_officer_first_name,
            attending_officer_middle_name,
            attending_officer_last_name,
    ...
    $stmt->execute([
        ':blotter_code' => generateBlotterCode($pdo), // Function to generate unique blotter code
        ...
        ':suspect_dob' => $_POST['suspect_dob'] ?? null,
        ':suspect_gender' => $_POST['suspect_gender'] ?? null,
        ':suspect_civil_status' => $_POST['suspect_civil_status'] ?? null,
    ...
    header('Location: ../../public/index.php?page=blotter-reports&success=1');
    exit;

} catch (Exception $e) {
    die("Failed to save blotter report: " . $e->getMessage());
}
